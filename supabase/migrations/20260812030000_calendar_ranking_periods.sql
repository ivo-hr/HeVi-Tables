-- Ranking periods follow the Europe/Madrid calendar: the current Monday-based
-- week and the current calendar month, rather than rolling 7/30-day windows.
...
as $$
  ...
  ranked_rows as (
    ...
    where t.group_id = p_group_id
      ...
      and case p_period
        when 'week' then
          coalesce(t.closed_date, t.created_at) >= (
            date_trunc('week', now() at time zone 'Europe/Madrid')
              at time zone 'Europe/Madrid'
          )
          and coalesce(t.closed_date, t.created_at) < (
            (date_trunc('week', now() at time zone 'Europe/Madrid') + interval '1 week')
              at time zone 'Europe/Madrid'
          )
        when 'month' then
          coalesce(t.closed_date, t.created_at) >= (
